...
const MAX_RETRIES = 3;
const LOG_FILE = path.join(__dirname, '..', 'logs', 'reprocess_narratives.log');
const PROGRESS_FILE = path.join(__dirname, '..', 'logs', 'reprocess_narratives_progress.json');
...
async function getNarrativesForReprocessing(lastProcessedID, limit) {
  try {
...
async function main() {
  ...
  log(`Starting narrative reprocessing from ID ${progress.lastProcessedID}`);
  ...
  while (running) {
    try {
      ...
      const batch = await getNarrativesForReprocessing(progress.lastProcessedID, BATCH_SIZE);
      ...
      if (batch.length === 0) {
        ...
        // Skip ahead by 1000 IDs to find more
        progress.lastProcessedID += 1000;
        saveProgress(progress);
        log(`Skipping ahead to ID ${progress.lastProcessedID}`);
