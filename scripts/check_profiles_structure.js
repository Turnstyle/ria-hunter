// Script to check ria_profiles table structure
...
async function checkRiaProfilesStructure() {
  try {
    // Fetch sample data from ria_profiles
    ...
    if (error) {
      console.error('Error querying ria_profiles:', error);
      return;
    }
    
    console.log('RIA Profiles table structure:');
    if (data && data.length > 0) {
      console.log('Columns:', Object.keys(data[0]));
      console.log('\nSample data:');
      console.log(JSON.stringify(data, null, 2));
    } else {
      console.log('No data found in ria_profiles table');
    }
  } catch (error) {
