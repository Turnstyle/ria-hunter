-- Migration: 20250115_ai_native_hybrid_search
-- Purpose: Replace hybrid_search_rias with AI-native implementation using RRF and fuzzy location handling

DROP FUNCTION IF EXISTS hybrid_search_rias(text, vector(768), float, integer, text, numeric, numeric);
DROP FUNCTION IF EXISTS hybrid_search_rias(text, vector(768), float, integer, text, numeric, numeric, numeric);
...
COMMENT ON FUNCTION hybrid_search_rias(TEXT, VECTOR(768), TEXT, TEXT, INTEGER, INTEGER) IS
'AI-native hybrid search using semantic + full-text RRF with fuzzy city/state matching';
